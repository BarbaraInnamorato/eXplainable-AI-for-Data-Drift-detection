...
def d3_xai(data_for_xai, cols, all_cols, filename):
    ...
    for diz in data_for_xai[-1:]:
        ...
        for i in range(len(diz['X_test'])):  # da 0 a 66-1
            ...
            print(f'D3 shap values {filename}', shap_values)
            print(f'D3 expected values {filename}', explainer_shap.expected_value)

            model_output = (explainer_shap.expected_value + shap_values[pred].sum()).round(4)
            #model_output = (explainer_shap.expected_value[pred] + shap_values[pred].sum()).round(4) # DA PROVAREEE
            ...
            plt.title(f'Local forceplot row {str(i)} dataset {filename}')
            plt.tight_layout()
            plt.savefig('images/'+ f'D3 Local forceplot row {str(i)} dataset {filename}')
            ...
            dizio = {'batch %s' % k: {'row %s' % i: {
                ...
                'ML_prediction': pred,
                'ML_pred_probs': predict_proba,             # D3 prediction (LR)
            ...
            lime_prediction = [round(max(lime_probs), 3), lime_probs.index(max(lime_probs))]
            ...
            lime_diz = {'batch %s' % k: {'row %s' % i: {
                ...
                'LIME_prediction': lime_prediction,                     # xai prediction
...
def st_xai(data_for_xai, cols, all_cols, filename):
    ...
    for diz in data_for_xai:
        ...
        print('ST model_output',model_output[:])

        class_pred = np.argmax(abs(model_output))
        print('ST class_pred',class_pred)
        #print('is ML pred == SHAP pred ?', pred == class_pred)
        ...
        dizio = {'batch %s' % k: {'row %s' % k: {
            'class_names': class_names,
            'ML_prediction': pred,
            'ML_pred_probs': predict_proba,             # ST prediction
        ...
        lime_diz = {'batch %s' % k: {'row %s' % k: {
            ...
            'ML_pred_probs': predict_proba,                     # ST predicted probs
            'LIME_LOCAL_prediction': exp_lime.local_pred,       # xai prediction
            'LIME_prediction': lime_prediction,                 # xai prediction
            'value_ordered': f_weight,                          # (feature, lime_weight)
            'feature_value': variables,                         # (feature, real value)
